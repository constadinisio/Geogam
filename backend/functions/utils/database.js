...
async function getUserFrame(userId) {
  const snapshot = await get(ref(db, `users/${userId}/equippedFrame`));
  // Provide a default frame if the user hasn't selected one
  return snapshot.exists() ? snapshot.val() : '/public/images/marco1.webp';
}

async function updateUserFrame(userId, frameUrl) {
  await set(ref(db, `users/${userId}/equippedFrame`), frameUrl);
}
